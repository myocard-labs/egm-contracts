...
ROLES_JSON = REPO_ROOT / "codegen" / "roles.json"
...
def _gen_roles(output_path: Path) -> None:
    """Emit the artifact-role vocabulary (Role enum + ROLE_PREFIXES) from roles.json.

    Hand-crafted emit rather than datamodel-code-generator, which produces types
    (models + enums) but not a module-level constant lookup table. Kept
    byte-deterministic (roles.json order preserved) so the codegen-drift check
    stays green. The role_of() logic over this vocabulary is hand-written in
    myocard_egm_contracts/roles.py.
    """
    ...
    lines = [
        ...
        "",
        "class Role(Enum):",
        *(f'    {r["name"]} = "{r["name"]}"' for r in roles),
        "",
        "",
        "ROLE_PREFIXES: dict[str, Role] = {",
        *(f'    "{r["prefix"]}": Role.{r["name"]},' for r in roles),
        "}",
        "",
    ]
    output_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"  roles.json -> {output_path.relative_to(REPO_ROOT)}")
...
def main() -> int:
    ...
    # The role vocabulary is single-sourced in codegen/roles.json and emitted
    # here (datamodel-code-generator produces types, not the constant prefix map).
    print("Generating role vocabulary...")
    _gen_roles(OUTPUT_DIR / "roles.py")
    ...
    init_lines.append("from . import roles  # noqa: F401")
    init_lines.append("")
